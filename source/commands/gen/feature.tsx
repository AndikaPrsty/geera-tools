	...
	const tasks = new Listr([
		{
			...
			async task() {
				const endpointPath = path.resolve(
					...
					'src/app/infrastructures/endpoints',
				);
		...
		},
		{
			...
			async task() {
				const apiRepositoryPath = path.resolve(
					projectDirectoryPath,
					'src/app/infrastructures/repositories/api',
				);
				await copyWithTemplate(
		...
		},
		{
			...
			async task() {
				const repositoryInterface = path.resolve(
					projectDirectoryPath,
					'src/data/persistences/repositories/contracts',
				);
				await copyWithTemplate(
					fromPath('RepositoryInterface.ts.tpl'),
					toPath(
						repositoryInterface,
						options.feature + 'RepositoryInterface.ts',
					),
					options.feature,
				);
			},
		...
		},
		{
			...
			async task() {
				const dIPath = path.resolve(
					projectDirectoryPath,
					'src/app/infrastructures/dependencies/modules',
				);
				await copyWithTemplate(
					fromPath('DI.ts.tpl'),
